...
int coloring(int a) {//暫時
	...
	//cout << endl;
	for (int i = 0; i < watermkGo.size(); i++) {
...
void connect(int a) {//開連
	...
	if (watermkGo[0].color == 'Y') {//Y開頭
						...
						for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
							...
							if (conflict == false) {//找到不會conflict的state
								...
								if (!hiding_B(a, our_currentState)) {//歐K
									cfCosts.insert(pair<string, int>(itbig->first, coloring(a) - mergeCost(our_currentState, our_nextState, newRoad, newRoad_output)));
									noStateHaveDC = false;
								}
								origin_vecState[our_currentState].erase(origin_vecState[our_currentState].find(newRoad));
								coloring(a);
		...
		ini[a].clear();
		//if(a == 0)
		//	ourStart_state = "S5";//find_state_to_makeTR(start_state, wm[a].watermark_input[0]);//找到一個可以新增第一個WM的STATE
		//else {
...
string find_state_to_makeTR(string currentState, string input) {//隨便找一個可以生transition的state
	...
	for (int i = 0; i < watermkGo.size(); i++) {//把黃包的結尾state要儲存下一包的input，避免conflict
		if (watermkGo[i].color == 'Y' && i != watermkGo.size()-1) {
			yelloEnd[watermkGo[i].lastState].push_back(watermkGo[i+1].state);
		}
	}
