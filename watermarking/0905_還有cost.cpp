...
map<string, map<string, NextNode> >::iterator nextIt_big(map<string, map<string, NextNode> >::iterator it) {
	it++;
	return it;
}
string line;
...
string find_state_to_makeTR(string, string, int);
char update_color(string, string);
void show_table();
...
void MergeState();
vector<string> merge_the_inputs(string);
...
bool isCSFSM(string state) {//state merge時用的，看去掉自己以外的STATE是不是CSFSM
	map<string, map<string, NextNode> >::iterator itbig;
	for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
		if (itbig->first != state) {
			if (isComplete(itbig->first) != "Complete") {
				return false;
			}
		}
...
string isComplete(string currentState) {//用vector存下這個state的所有input，用來merge與比較
	vector<string> inputs = merge_the_inputs(currentState);
	...
	if (inputs.size() == 0) {//size = 0 直接有DC
		return counter;
	}
	else if (inputs[0] == counter_fullDC) {//全都是---------
		return "Complete";
	}
	else {//用智慧counter數
		  //先找到最左邊的全dc與最右邊的全dc   ex: ---01011-- 與-00111----  則刪左1右2
		int DCNum = 0;
		int smallLNum = 999999;
		int smallRNum = 999999;
		//左邊
		...
		//先把左右兩邊的dc去掉
		string cutinput;
		vector<string> cutinputs;
		for (int i = 0; i < inputs.size(); i++) {
			for (int j = smallLNum; j < inputs[i].size() - smallRNum; j++) {
				cutinput += inputs[i][j];
			}
			cutinputs.push_back(cutinput);
			cutinput.clear();
		}
		//初始化counter的bit
		int new_bitsize = input_bit - (smallLNum + smallRNum);
		counter.resize(new_bitsize);
		counter.assign(new_bitsize, '0');
		counter_end.resize(new_bitsize);//設置courter大小 = .i
		counter_end.assign(new_bitsize, '0'); //111 + 1 ->000 
		
		//看看有沒有左邊全是'0'的右邊全是'-',如果有:把counter再初始化成下一個 ex:0---- 從10000開始 00-- 從0100
		int LastZero = -1;//存最後一個0的位置
		bool quit = false;// 如果有'1'不可以到下面的if
		int smallestLZ = 999999; // 比哪一個counter好
		for (int i = 0; i < cutinputs.size(); i++) {	
			quit = false;
			for (int j = 0; j < cutinputs[i].size(); j++) {
				if (cutinputs[i][j] == '1') {//有1就break ,counter還是從00000000開始
					quit = true;
					break;
				}
				else if(cutinputs[i][j] == '0'){
					LastZero = j;
				}
			}
			if (!quit) {//沒有'1'可以改成更好counter
				if (smallestLZ > LastZero) {
					smallestLZ = LastZero;
				}
			}

		}
		if (smallestLZ != 999999) {
			counter[smallestLZ] = '1';
		}
		bool start_loop = true;
		//並開始數
		int i = 0;
		while (counter != counter_end || start_loop) {//if (counter == 1111111) --> 比下一組 //start_loop 讓000=000可以進來
			if (isINCLUDE(counter, cutinputs[i])) {//相等
				...
				i++;
				if (i == cutinputs.size() ) {
					for (int k = 0; k < smallLNum; k++) {
						counter = '0' + counter;
					}
					for (int k = 0; k < smallRNum; k++) {
						counter += '0';
					}
					return counter;//有dont care 所以不是complete//結束，數完後回傳counter
				}
			}
		}
		return "Complete";
...
vector<string> merge_the_inputs(string currentState) {
	vector<string> inputs;
	map<string, NextNode>::iterator it = origin_vecState[currentState].begin();
	for (it; it != origin_vecState[currentState].end(); it++) {//用vector存下這個state的所有input
	...
	while (i < inputs.size()) {
		for (int j = i + 1; j < inputs.size(); j++) {
			...
			for (int k = 0; k < inputs[i].size(); k++) {
				if (inputs[i][k] == inputs[j][k]) {//這個bit相等 ex: 00"0" / 01"0"
					newkey += inputs[i][k];
					mergekey += inputs[i][k];
				}
				else if (inputs[i][k] == '-') {//任意一個是don't care
					newkey += '-';
					string oppo = "0";
					oppo[0] = inputs[j][k];
					...
					newkey += '-';
					diffconuter++;
					mergekey += inputs[i][k];//存好上面的 DCconuter==0
					diff_position = k;
				}
			}
			...
			if ((DCconuter == 1 && diffconuter == 0 && DCconuter2 == 1) || (DCconuter == 1 && diffconuter == 1 && DCconuter2 == 0) || (DCconuter == 0 && diffconuter == 1 && DCconuter2 == 1)) {
				if ((DCconuter == 1 && diffconuter == 1 && DCconuter2 == 0) || (DCconuter == 0 && diffconuter == 1 && DCconuter2 == 1)) {//有diff
					if (DCconuter == 1) {
						mergekey[diff_position] = inputs[j][diff_position];
					}
				}
				for (int k = 0; k < inputs.size(); k++) {
					//找到一個是包含mergekey/相同的input路
					if (isINCLUDE(mergekey, inputs[k])) {
			...
			//只能是一個不同才可以合併 就可合
			if ((diffconuter == 1 && DCconuter == 0 && DCconuter2 == 0) || (DCconuter == 0 && diffconuter == 0) || (DCconuter2 == 0 && diffconuter == 0) || findCanMerge) {//能合 更新table
				inputs[i] = newkey;//i吃掉j
...
string counter_add(string counter) {//棒棒
...
void MergeState() {
	map<string, map<string, NextNode> >::iterator itbig = origin_vecState.begin();
	map<string, map<string, NextNode> >::iterator itbig2 = origin_vecState.begin();
	map<string, bool > same_key;//如果KEY一樣就要先存下來免得等等要拔，因為KEY一樣會蓋過去
								//只要任意2個state出去的路沒有衝突
	bool add = true;
	while (itbig != origin_vecState.end()) { //跑每個state
		add = true;
		for (itbig2 = origin_vecState.begin(); itbig2 != origin_vecState.end(); itbig2++) {
			if (itbig2->first != itbig->first) {//去掉自己
				same_key.clear();
				map<string, NextNode>::iterator it2 = origin_vecState[itbig2->first].begin();//第二個
				int pass = 0;
				//如果是空的,則一定可以合
				for (it2; it2 != origin_vecState[itbig2->first].end(); it2++) {
					if (find_input_in_map(itbig->first, it2->first) != "not_Found") {// == not_Found -->沒有這個input
						if (it2->second.output != origin_vecState[itbig->first].find(find_input_in_map(itbig->first, it2->first))->second.output ||
							(it2->second.next_state != origin_vecState[itbig->first].find(find_input_in_map(itbig->first, it2->first))->second.next_state
								&& (it2->second.next_state != itbig->first || origin_vecState[itbig->first].find(find_input_in_map(itbig->first, it2->first))->second.next_state != itbig2->first)))
						{//回傳input,如果output一樣=>沒conflict , != 所以break , 如果沒有回到自己的話也不算
							break;
						}
						else if (it2->first == origin_vecState[itbig->first].find(find_input_in_map(itbig->first, it2->first))->first) {//如果一樣可能會蓋過去,存下
							same_key.insert(pair<string, bool>(it2->first, true));
						}
						else {
							same_key.insert(pair<string, bool>(it2->first, false));
						}
					}
					pass++;
				}

				if (pass == origin_vecState[itbig2->first].size()) { //能合
																	 //先copy S1內的東西到S0
					for (it2 = origin_vecState[itbig2->first].begin(); it2 != origin_vecState[itbig2->first].end(); it2++) {
						//如果一樣的tr不用insert
						origin_vecState[itbig->first].insert(pair<string, NextNode>(it2->first, it2->second));
					}
					//看看會不會CSFSM 如果是,要把剛插上去的拿掉big

					if (isCSFSM(itbig2->first)) {//傳入不要看的state
						for (it2 = origin_vecState[itbig2->first].begin(); it2 != origin_vecState[itbig2->first].end(); it2++) {
							//如果一樣的tr不用insert
							if (same_key[it2->first] == false)
								origin_vecState[itbig->first].erase(it2->first);
						}
					}
					else {
						// 再把所有S1變成S0
						//cout << itbig->first << " " << itbig2->first << endl;
						map<string, map<string, NextNode> >::iterator itbig3 = origin_vecState.begin();
						for (itbig3; itbig3 != origin_vecState.end(); itbig3++) {
							map<string, NextNode>::iterator it = origin_vecState[itbig3->first].begin();
							for (it = origin_vecState[itbig3->first].begin(); it != origin_vecState[itbig3->first].end(); it++) {
								if (itbig3->first != itbig2->first) {//S1不用變自己
									if (it->second.next_state == itbig2->first) { //如果有其他S1的,變成S0
										it->second.next_state = itbig->first;
									}
								}
							}
						}
						//把itbig->first是S1刪掉
						origin_vecState.erase(itbig2->first);
						//減數,指回去
						state_num--;
						add = false;
						itbig = origin_vecState.begin();//重新做合
						break;
					}
				}
			}
				...
				if (it->second.next_state == it2->second.next_state && it->second.output == it2->second.output) {
					...
					if ((DCconuter == 1 && diffconuter == 0 && DCconuter2 == 1) || (DCconuter == 1 && diffconuter == 1 && DCconuter2 == 0) || (DCconuter == 0 && diffconuter == 1 && DCconuter2 == 1)) {
						...
						for (it3 = origin_vecState[itbig->first].begin(); it3 != origin_vecState[itbig->first].end(); it3++) {
							...
							}//因為Mergekey包含太多，檢查會不會包到現有的 // 去掉dont
							else if (diff(it3->first, mergekey) == 0) {//diffcounter > 0代表沒有互相包含一些，所以=0代表不能加進來，因為這個mergekey包含到這個input的某些元素
								findCanMerge = false;
								finddontMerge = false;
							}
						}
...
int coloring(int a, bool count_cost) {//暫時
	...
	//cout << endl;
	/*for (int i = 0; i < watermkGo.size(); i++) {//reuse score ，但結果不好
	if (watermkGo[i].color == 'Y')
	cfCost = cfCost - watermkGo[i].size;
	}*/
	if(count_cost)
		cfCost += precolor(a);
...
int precolor(int b) {//傳入a以預塗a以後的WM
	vector<WatermarkStyle>watermkGo_precolor;
	int cfCost = 0;//計算 Y-->R 和 R-->B 的數量 (conflict cost
	for (int a = b+1; a < 3; a++) {//a=0的話就塗1跟2
		int i = 0;
		watermkGo_precolor.clear();
		while (i < wm[a].watermark_input.size()) {//走完wm
			vector<WatermarkStyle> roads;//存下每個wm_input,output多少組可走的路
			...
			if (roads.size() == 0) {//白色and藍色and紅色
				...
				WStemp.lastState = "S-1";
				WStemp.size = 1;
				watermkGo_precolor.push_back(WStemp);//放入一個東西
				i++;
			}
			else {//一包的ck
				WatermarkStyle WSmin;//初始值
				...
				WSmin.lastState = "S-1";
				WSmin.size = 0;
				watermkGo_precolor.push_back(WSmin);//先放最小值,才可以比較出roads的大小(找出最長string)
						...
						if (i_ptr < wm[a].watermark_input.size()) {//防爆(超出範圍)
							...
							/*else*/if (!findInput) {//斷開
								...
								if (isConflict) {//跳出while(1)不要存    //如果全conflict跳到if (countbreak == roads.size()) 
									...
									if (watermkGo_precolor[watermkGo_precolor.size() - 1].size < roads[j].size) {
										watermkGo_precolor.pop_back();
										watermkGo_precolor.push_back(roads[j]);
									}
									else if (watermkGo_precolor[watermkGo_precolor.size() - 1].size == roads[j].size && rand() % (rand() % 2 + 2) != 1) {/////////////////////////////////
										watermkGo_precolor.pop_back();
										watermkGo_precolor.push_back(roads[j]);
				...
				i = watermkGo_precolor[watermkGo_precolor.size() - 1].size + i;//i到包尾的下一個
			}
			//cout << "watermark[" << watermkGo_precolor[watermkGo_precolor.size() - 1].current_step << "]: " << wm[a].watermark_input[watermkGo_precolor[watermkGo_precolor.size() - 1].current_step] << " " << wm[a].watermark_output[watermkGo_precolor[watermkGo_precolor.size() - 1].current_step] << endl;
			//cout << watermkGo_precolor[watermkGo_precolor.size() - 1].color << " Step:" << watermkGo_precolor[watermkGo_precolor.size() - 1].current_step << " Size:" << watermkGo_precolor[watermkGo_precolor.size() - 1].size << " " << watermkGo_precolor[watermkGo_precolor.size() - 1].state << " last: " << watermkGo_precolor[watermkGo_precolor.size() - 1].lastState << endl;
		}
	}
	return cfCost;
}
...
void connect(int a) {//開連
	...
	if (watermkGo[0].color == 'Y') {//Y開頭
		if (watermkGo.size() == 1) {//WM已經都在FSM裡面了(只剩一包黃色)
			...
			if (a == 2) {
				if ((smallMdCost > (transition_num + state_num))) {//第一次做完，在MERGE前先寫一次檔
				...
				if (transition_num + state_num == 109) {
					cost49++;
				}
				if (transition_num + state_num <= 108) {
					cost48++;
				}
						...
						for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
							...
							if (conflict == false) {//找到不會conflict的state
								...
								if (!hiding_B(a, our_currentState)) {//歐K
									cfCosts.insert(pair<string, int>(itbig->first, coloring(a,true) - mergeCost(our_currentState, our_nextState, newRoad, newRoad_output)));
									noStateHaveDC = false;
								}
								origin_vecState[our_currentState].erase(origin_vecState[our_currentState].find(newRoad));
								coloring(a,false);
		...
		string newRoad_output = wm[a].watermark_output[0];

		map<string, NextNode>::iterator it;
		vector<string> dontcares_input;
		vector<string> dontcares_output;
		for (int b = a; b < 3; b++) {
			for (int i = 0; i < wm[b].watermark_input.size(); i++) {
				bool isDC = true;
				for (it = origin_vecState[ourStart_state].begin(); it != origin_vecState[ourStart_state].end(); it++) {
					if (isINCLUDE(wm[b].watermark_input[i], it->first)) {
						isDC = false;
						break;
					}
				}
				if (isDC) {
					dontcares_input.push_back(wm[b].watermark_input[i]);
					dontcares_output.push_back(wm[b].watermark_output[i]);
				}
			}
		}
		if (dontcares_input.size() != 0) {
			int r = rand() % dontcares_input.size();
			newRoad = dontcares_input[r];
			newRoad_output = dontcares_output[r];
		}
		...
		ini[a].clear();
		ourStart_state = find_state_to_makeTR(start_state, wm[a].watermark_input[0], a);//找到一個可以新增第一個WM的STATE
		our_currentState = ourStart_state;
...
string find_ourStart_state_withDC(string currentState) {//找到最近的一個有dont care的state
	...
	for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
		if (isComplete(itbig->first) != "Complete") {
			StartStates.push_back(itbig->first);
		}
	}
	return StartStates[rand() % StartStates.size()];
}

string find_state_to_makeTR(string currentState, string input, int a) {//隨便找一個可以生transition的state
	map<string, vector<string> > yelloEnd;//計算用過的路的數量
	map<string, map<string, NextNode> >::iterator itbig;
	for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
		vector<string> to_leave_DC;
		yelloEnd.insert(pair<string, vector<string> >(itbig->first, to_leave_DC));
	}
	for (int i = 0; i < watermkGo.size(); i++) {//把黃包的結尾state要存下一包的input，避免conflict
		if (watermkGo[i].color == 'Y' && i != watermkGo.size() - 1) {
			yelloEnd[watermkGo[i].lastState].push_back(wm[a].watermark_input[watermkGo[i + 1].current_step]);
		}
	}

	//找一個可以生transition的state
	vector<string> StartStates;//存著所有能當Start的State
	vector<string> bad_StartStates;
	map<string, NextNode>::iterator it;
	for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {//看所有state
		...
		if (have_dontcare) {//這個state有這個input的dont care可以加，接下來就檢查它會不會害其他Y不能用
			if (yelloEnd[itbig->first].size() != 0) {//有可能會害其他Y不能用
													 /*int conflict = 0;
													 for (int i = 0; i < yelloEnd[itbig->first].size(); i++) {
													 if (input == yelloEnd[itbig->first][i]) {
													 conflict++;
													 }
													 }
													 if (conflict != 0) {//會害其他Y不能用，另外存
													 if (conflict < min_conflict) {
													 bad_StartStates.clear();
													 min_conflict = conflict;
													 bad_StartStates.push_back(itbig->first);
													 }
													 else if (conflict == min_conflict) {
													 bad_StartStates.push_back(itbig->first);
													 }
													 }
													 else {//檢查後不會害其他Y不能用，直接存
													 StartStates.push_back(itbig->first);
													 }*/
				vector<string>::const_iterator v = find(yelloEnd[itbig->first].begin(), yelloEnd[itbig->first].end(), input);
				if (v != yelloEnd[itbig->first].end()) {//會害其他Y不能用，另外存
					bad_StartStates.push_back(itbig->first);
				}
				else {//檢查後不會害其他Y不能用，直接存
					StartStates.push_back(itbig->first);
			...
			else {//不用擔心會害其他Y不能用，直接存
				StartStates.push_back(itbig->first);
			}
		}
	}
	if (StartStates.size() != 0) {//有好的state可以選就選
		return StartStates[rand() % StartStates.size()];
	}
	else {
		cout << "-----------------------------" << endl;
		return bad_StartStates[rand() % bad_StartStates.size()];
	}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		/*map<string, map<string, NextNode> >::iterator itbig;
		for (itbig = origin_vecState.begin(); itbig != origin_vecState.end(); itbig++) {
			vector<string> inputs = merge_the_inputs(itbig->first);
			cout << itbig->first << ": ";
	...
	cout << endl;
	cout << "best count: " << best_count << "  cost49: " << cost49 << "  cost48: " << cost48 << endl;
